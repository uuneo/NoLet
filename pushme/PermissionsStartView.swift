...
struct PermissionsStartView: View {
        ...
        VStack(spacing: 20) {
                            ...
                            HStack {
                                ...
                                TextField("请输入服务器地址", text: $customServerAddress)
                                    ...
                                    .onChange(of: customServerAddress) { newValue in
                                        appManager.customServerURL = newValue
                                    }
                            }
                            .padding(.vertical, 8)
                            .padding(.horizontal, 15)
            ...
            VStack(spacing: 15) {
                if currentStep == .networkPermission {
                    ...
                    Button {
                        if let networkIndex = permissionOptions.firstIndex(where: { $0.mode == .network }) {
                            if permissionOptions[networkIndex].isSelected {
                                // 检查如果开启了自定义服务器，需要验证URL
                                if useCustomServer {
                                    if PermissionsStartView.isValidURL(customServerAddress) {
                                        // URL有效，保存并进入下一步
                                        ...
                                        // URL无效，显示警告
                                        alertTitle = "服务器地址无效"
                                        alertMessage = "请输入有效的服务器URL地址，例如：https://example.com"
                                        showAlert = true
                                ...
                                // 网络权限未选择，显示警告
                                alertTitle = "需要网络权限"
                                alertMessage = "网络权限是应用正常运行的基础，请授予网络权限以继续。"
                                showAlert = true
                            }
                        }
                    } label: {
                    ...
                    Button {
                        if requiredPermissionsSelected {
                            ...
                            // 显示警告
                            alertTitle = "必选权限未选择"
                            alertMessage = "普通通知权限为必选项。如果开启重要通知权限，所有权限将自动开启。请选择必要的权限以继续使用应用。"
                            showAlert = true
                        }
                    } label: {
                        Text("完成设置")
    ...
    init(complete: (() -> Void)? = nil) {
        ...
        // 设置默认的alert标题和消息
        _alertTitle = State(initialValue: "需要网络权限")
        _alertMessage = State(initialValue: "网络权限是应用正常运行的基础，请授予网络权限以继续。")
    }
